...
def evaluate_probabilistic(y_true_binary, y_true_returns, probabilities,
                            ensemble_returns_list, horizon_label=""):
    """
    Full probabilistic evaluation suite.
    Combines legacy classification metrics with proper scoring rules.
    ...
    """
    from prepare import evaluate_predictions

    # Legacy classification metrics (unchanged — results.tsv stays consistent)
    class_metrics = evaluate_predictions(y_true_binary, probabilities,
...
def run_experiment(experiment_name=None, verbose=1):
    ...
    metrics = evaluate_probabilistic(
        ...
        all_ensembles, horizon_label=PROJECTION_HORIZON
    )
