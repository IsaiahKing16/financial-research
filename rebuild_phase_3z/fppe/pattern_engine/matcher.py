...
from rebuild_phase_3z.fppe.pattern_engine.contracts.signals import NeighborResult
from rebuild_phase_3z.fppe.pattern_engine.features import apply_feature_weights
...
class PatternMatcher:
    ...
    def _post_filter(
        ...
        if X_batch is not None and self._sax_filter is not None:
            from rebuild_phase_3z.fppe.pattern_engine.sax_filter import apply_sax_filter
    ...
    def fit(
        ...
        if getattr(cfg, 'use_ib_compression', False):
            from rebuild_phase_3z.fppe.pattern_engine.ib_compression import IBCompressor
        ...
        if getattr(cfg, 'use_sax_filter', False):
            from rebuild_phase_3z.fppe.pattern_engine.sax_filter import SAXFilter
        ...
        if getattr(cfg, 'use_wfa_rerank', False):
            from rebuild_phase_3z.fppe.pattern_engine.wfa_reranker import WFAReranker
