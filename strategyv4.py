...
def _run_matching_loop(train_db, val_db, scaler, feature_cols, verbose=1,
    ...
    for idx in range(len(val_db)):
        ...
        # Default args are bound at function definition time, so runtime global
        # mutations would NOT affect generate_signal() if called bare.
...
# ============================================================
# PARAMETER SWEEP (added after baseline analysis 2026-03-16)
# ============================================================
...
def run_analogue_sweep(budget_minutes=60, verbose=1):
    ...
    MOTIVATION (from baseline analogue_v1_baseline):
      - BSS = -0.031: probability outputs worse than climatology
      - avg_matches = 50.0: distance filter never binding (MAX_DISTANCE too loose)
      - 8,734 trades (66%): CONFIDENCE_THRESHOLD too permissive
      - 7,116 BUY vs 1,618 SELL: bullish bias from loose matching

    PRIMARY RANKING METRIC: Brier Skill Score (BSS)
      BSS > 0 = beats base rate probability forecast
      BSS > 0.05 = materially useful
      Then: confident-trade accuracy, then coverage

    WHAT WE SWEEP:
      MAX_DISTANCE    — how similar analogues must be (key fix: tighten from 0.5)
      TOP_K           — max analogues per query (fewer, higher-quality)
      SAME_SECTOR_ONLY — sector-cohort vs full-universe matching
      DISTANCE_WEIGHTING — inverse vs uniform weighting
      CONFIDENCE_THRESHOLD — selectivity control
      AGREEMENT_SPREAD — consensus requirement

    Results go to results_analogue.tsv (separate from legacy neural-net results.tsv).
    Each run uses the full validation set so BSS is comparable across experiments.
    """
    import time

    # Sweep grid — ordered by expected impact based on baseline diagnostics
    # Tier 1: distance tightening (most likely to fix BSS)
    # Tier 2: K reduction (fewer, better matches)
    # Tier 3: sector/weighting variants
    # Tier 4: threshold tuning on best distance config

    configs = [
        # Tier 1: Tighten MAX_DISTANCE — the primary fix for avg_matches=50.0
        {"name": "sweep_dist0.3_k50",  "MAX_DISTANCE": 0.3,  "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},
        {"name": "sweep_dist0.2_k50",  "MAX_DISTANCE": 0.2,  "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},
        {"name": "sweep_dist0.35_k50", "MAX_DISTANCE": 0.35, "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},

        # Tier 2: Reduce K — fewer but higher-quality analogues
        {"name": "sweep_dist0.3_k20",  "MAX_DISTANCE": 0.3,  "TOP_K": 20,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},
        {"name": "sweep_dist0.3_k25",  "MAX_DISTANCE": 0.3,  "TOP_K": 25,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},

        # Tier 3: Sector cohort and weighting variants
        {"name": "sweep_sector_d0.3",  "MAX_DISTANCE": 0.3,  "TOP_K": 50,  "SAME_SECTOR_ONLY": True,  "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},
        {"name": "sweep_uniform_d0.3", "MAX_DISTANCE": 0.3,  "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "uniform", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.10},

        # Tier 4: Threshold tuning on best-expected distance (0.3)
        {"name": "sweep_thr60_d0.3",   "MAX_DISTANCE": 0.3,  "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.60, "AGREEMENT_SPREAD": 0.10},
        {"name": "sweep_thr65_d0.3",   "MAX_DISTANCE": 0.3,  "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.65, "AGREEMENT_SPREAD": 0.10},
        {"name": "sweep_agree15_d0.3", "MAX_DISTANCE": 0.3,  "TOP_K": 50,  "SAME_SECTOR_ONLY": False, "DISTANCE_WEIGHTING": "inverse", "CONFIDENCE_THRESHOLD": 0.55, "AGREEMENT_SPREAD": 0.15},
    ]
    ...
    for i, cfg in enumerate(configs):
        ...
        # This is the correct pattern: _run_matching_loop now accepts all
        # sweep parameters directly, so there is no __main__/imported-module
        # split and no default-argument-binding surprise.
        try:
...
if __name__ == "__main__":
    ...
    # Run after a promising sweep result to verify regime stability.
    ...
    # Run this next. Addresses the BSS=-0.031 and avg_matches=50.0 issues.
    # Budget: 60 min. Results go to results/results_analogue.tsv.
    # Rank by BSS first — that's the metric that matters now.
    run_analogue_sweep(budget_minutes=60)

    # ── MODE 4: Best sweep config → walk-forward ──
    # After sweep identifies the best distance/K/sector config,
    # apply those params above and run walk-forward to test regime stability.
