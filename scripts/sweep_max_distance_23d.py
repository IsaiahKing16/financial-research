...
The 8D optimum was max_distance=0.90. Adding 15 candlestick dimensions inflates
Euclidean distances by √(23/8) ≈ 1.7×. Almost no analogues pass the original
threshold, producing all-HOLD signals and meaningless BSS.

This sweep finds the WINNER: the smallest max_distance where ALL 6 walk-forward
folds have AvgK ≥ 20, using the returns_candle (23D) feature set on the 52T universe.

Strategy: for each fold, fit the matcher ONCE at max_distance=3.0 (the largest
sweep value), then sweep by mutating cfg.max_distance. This reduces 54 fit+query
operations to 6 fits + 54 queries. Calibrator miscalibration at non-fitted
thresholds is acceptable here — the primary gate criterion is AvgK, not BSS.
BSS values are informational; Task 6.2 does a clean full fit+query per config.

Locked settings applied (CLAUDE.md):
    calibration=beta_abm, max_distance swept, top_k=50, uniform weighting,
    regime=hold_spy_threshold=+0.05 (H7 HOLD mode), nn_jobs=1

Provenance: HANDOFF_Phase6-remainder_Phase7.md, Task 6.1

Usage:
    PYTHONUTF8=1 py -3.12 scripts/sweep_max_distance_23d.py

Output:
    results/phase6/sweep_max_distance_23d.tsv
"""
...
# ── Sweep values: brackets the 8D→23D scaling estimate of ~1.53 ───────────────
# 8D optimum was 0.90; Euclidean scales as O(√d), so 23D ≈ 0.90 × √(23/8) ≈ 1.53.
# Values below 0.9 are sanity checks (expected to produce AvgK ≈ 0).
MAX_DISTANCE_VALUES = [0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 2.0, 2.5, 3.0]
...
class _BetaCalibrator:
    ...
    This is the same mechanism used in scripts/experiments/h7_regime_filter.py.
    """
...
class _ThresholdRegimeLabeler:
    ...
    Copied from h7_regime_filter.py (authoritative for H7 regime logic).
    """
...
# ── Candlestick augmentation (mirrors smoke_test_candle_wf.py) ────────────────
...
def main() -> None:
    ...
    print("=" * 72)
    print("  Task 6.1 — max_distance sweep, returns_candle (23D), 52T universe")
    print("=" * 72)
    ...
    # ── Walk-forward sweep ────────────────────────────────────────────────────
    # Strategy: fit matcher ONCE per fold at max_distance=3.0, then mutate the
    # threshold for each sweep value. This avoids 9× redundant fits.
    # Calibrator is fitted at 3.0; BSS values at other thresholds carry slight
    ...
    try:
        for fi, fold in enumerate(WALKFORWARD_FOLDS):
            ...
            if len(val_db) == 0:
                for d in MAX_DISTANCE_VALUES:
                    rows.append({
                        ...
                        "n_scored": 0, "n_hold": 0, "pct_hold": 1.0,
                        "runtime_s": 0.0,
                    })
                continue
            ...
            # Fit at max_distance=3.0 (calibrator sees maximum analogue pool)
            cfg = EngineConfig()
            cfg.max_distance        = 3.0
            cfg.top_k               = 50
            ...
            print(f"  {'d':>5}  {'AvgK':>6}  {'BSS':>9}  "
                  f"{'n_scored':>8}  {'n_hold':>6}  {'%hold':>6}  {'t(s)':>5}")
            print(f"  {'-'*60}")
            ...
            for d in MAX_DISTANCE_VALUES:
                ...
                # Degenerate fold detection — only at largest value
                if avg_k < 1.0 and d >= 3.0:
                    print(f"  [WARNING] AvgK={avg_k:.3f} at max_distance=3.0 on fold "
                ...
                # n_hold = matcher-issued HOLD + regime-forced HOLD (bear rows that
                # got BUY/SELL from matcher but are overridden by H7 logic)
                ...
                pct_hold       = n_hold_total / len(sigs)

                print(f"  {d:>5.1f}  {avg_k:>6.1f}  {bss_val:>+9.5f}  "
                      f"{n_scored:>8}  {n_hold_total:>6}  {pct_hold:>6.1%}  "
                      f"{elapsed:>5.0f}")
    ...
    # ── Write TSV ──────────────────────────────────────────────────────────────
    tsv_path = RESULTS_DIR / "sweep_max_distance_23d.tsv"
    df_out = (
        pd.DataFrame(rows)
    ...
    if winner is not None:
        ...
        print(f"  Next: run Task 6.2 (BSS comparison) using this as the 23D threshold.")
        print(f"  Provenance: {tsv_path}")
    else:
        print(f"\nNO WINNER found in sweep range.")
        print("  No max_distance value achieves AvgK ≥ {AVGK_GATE} on all 6 folds.")
        print("  Consider extending the sweep above 3.0 or investigating data quality.")
